...
//#define TXE_PIN RPI_V2_GPIO_P1_24  // Version 1
#ifdef PI_GATE_868
#define TXE_PIN RPI_V2_GPIO_P1_13  // Pi-Gate
#endif

// Interrupt vectors for the 3 Arduino interrupt pins
// Each interrupt can be handled by a different instance of RH_SX1276, allowing you to have
// 2 or more LORAs per Arduino

#ifndef RH_SX1276_IRQLESS
RH_SX1276* RH_SX1276::_deviceForInterrupt[RH_SX1276_NUM_INTERRUPTS] = {0, 0, 0};
uint8_t RH_SX1276::_interruptCount = 0; // Index into _deviceForInterrupt for next device
#endif
...
RH_SX1276::RH_SX1276(uint8_t slaveSelectPin, uint8_t interruptPin, RHGenericSPI& spi)
    :
    RHSPIDriver(slaveSelectPin, spi),
    _rxBufValid(0)
{
  // modified ex-store.de
//  _resetPin = RPI_V2_GPIO_P1_22;
  // end ex-store.de
#ifndef RH_SX1276_IRQLESS
...
bool RH_SX1276::init()
{
  ...
  // TXE Pin modified ex-store.de
#ifdef PI_GATE_868
  pinMode(TXE_PIN, OUTPUT);   // Pi-Gate
  digitalWrite(TXE_PIN, LOW); // Pi-Gate
#endif
  ...
  if (version != 0x12) {// sx1272
    ...
    if (version != 0x22) {// sx1276
 			printf("Kein 0x12 oder 0x22 device gefunden. Version ist 0x%02x\n", version);
      return false;;
    }
  }
  ...
  // end ex-store.de
	...
	if (_interruptCount <= RH_SX1276_NUM_INTERRUPTS)
...
#ifndef RH_SX1276_IRQLESS
void RH_SX1276::handleInterrupt()
{
    // Read the interrupt register
    uint8_t irq_flags = spiRead(RH_SX1276_REG_12_IRQ_FLAGS);
    if (_mode == RHModeRx && irq_flags & (RH_SX1276_RX_TIMEOUT | RH_SX1276_PAYLOAD_CRC_ERROR))
    {
	_rxBad++;
    }
    else if (_mode == RHModeRx && irq_flags & RH_SX1276_RX_DONE)
    {
    ...
    else if (_mode == RHModeTx && irq_flags & RH_SX1276_TX_DONE)
    {
	_txGood++;
	setModeIdle();
    }
